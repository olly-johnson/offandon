...
- **Fathom Engine (`src/engines/fathom`):** Webhook-driven ingestion of Fathom recordings. Verifies HMAC, resolves the user by intersecting invitee emails with `auth.users` (skipping operator emails), then an Inngest function fetches the full transcript via Fathom's REST API and writes through the Corpus Engine. Idempotent by `source_path = fathom://<recording_id>`. Surface: `/transcripts` (list + detail).
