 ...
 * a Hole", "The Lesson", etc.). The LLM extraction path (BO-042) deliberately
 * skipped the `scripts/` directory, so until now these never made it
 * into `client_assets` as past_script rows.
 *
 * This module parses every `.md` file under `scripts/` deterministically,
 * pulls the framework + a few other useful fields out of the frontmatter,
 * and emits `ExtractedClientAsset` rows ready to be appended to the
 * ingestion pipeline's `.extracted.json`. No LLM call needed — the
 * structure is consistent enough to regex.
 *
 * Why deterministic instead of LLM? Three reasons:
 ...
 * downstream loader (BO-053 step 2) can group past_scripts by it.
 */
...
function humaniseRelativePath(rel: string): string {
  ...
  // Preserve apostrophes ("hero's_journey" -> "hero's journey"); replace
  // underscores/dashes/slashes with spaces.
