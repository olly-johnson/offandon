    ...
    <main className="mx-auto max-w-3xl px-4 py-12">
      ...
      {batch.status === "failed" ? (
        <section className="mb-6 rounded-lg border border-destructive bg-destructive/10 p-4 text-sm">
          ...
          <p className="mt-1 text-muted-foreground">{batch.failure_reason ?? "Unknown error."}</p>
        </section>
      ) : null}

      {inFlight ? (
        <section className="mb-6 rounded-lg border border-border bg-card p-4 text-sm text-muted-foreground">
          Claude is generating {batch.count_requested} scripts. Page refreshes automatically every
          few seconds.
        </section>
      ) : null}
