...
| Task ID | Description | Owner | Status | Branch / PR |
| :--- | :--- | :--- | :--- | :--- |
| BO-076 | Capture + visualise weekly growth from the check-in. New nullable numeric columns on `weekly_checkins` (new_followers, dms_received, calls_booked, sales_closed, leads_generated, revenue, posts_published, satisfaction; migration `20260531000000_checkin_metrics.sql`, satisfaction 1-10 CHECK). Pure `extractCheckinMetrics` + tolerant `parseStatNumber`/`sumStatNumbers` (currency/commas/k-m/"7 new DMs"; posts summed across platforms; "leads" plural so revenue's "leading" can't collide; satisfaction clamped) runs in both the GHL and Google-Form webhooks via an extended `saveCheckin`. Read side: `listRecentCheckinMetrics` + pure `buildWeeklyProgress` (latest/previous/delta/series per metric, revenue excluded). New dashboard "Your Weekly Progress" card (per-metric tile + delta badge + SVG sparkline), shown after Engagement when `hasData`. One-off `npm run checkin:backfill` (scripts/backfill-checkin-metrics.ts) re-parses `raw_responses` on existing rows (--dry-run / --force). TDD: metrics extractor (13) + progress builder (7). | claude | In Progress | feature/ghl-checkin-webhook |
