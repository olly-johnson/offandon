...
| BO-055 | Script library: delete action. `deleteScriptForUser` persistence helper (scoped to id+user_id), `deleteScriptAction` server action, inline two-stage confirm + trash icon on each library row. Vitest covers the helper. | claude | In Progress | feature/delete-scripts |
| BO-056 | Funnel-chart wiring fix. `scripts` table currently has no `angle` or `pillar` column, so the dashboard's Trust Funnel Balance chart is empty for every user. Add columns + populate at `saveGeneratedScripts` + `saveSingleScript`, drop the hardcoded `angle: null` in `dashboard/data.ts:70-78`. | - | Todo | - |
