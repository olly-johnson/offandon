...
| BO-042 | Operator-driven client ingestion: read `clients/<slug>/` files, LLM-extract into `voice_dna` + `client_assets` + `user_memories` + `user_methodology`. Two CLI commands (extract → review → commit). Bypass wizard when voice_dna already populated. | claude | In Progress | feature/client-ingestion |
