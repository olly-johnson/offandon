...
| BO-053 | Framework-keyed past_script retrieval. Deterministic parser for `clients/<slug>/scripts/**/*.md` reads `Framework:` frontmatter into `metadata.framework`. `loadScriptAssetsContext` groups past_scripts so one example per framework lands in the prompt (cap 6). SingleScriptGenerator accepts an optional `framework` filter so the wizard pulls only the matching example. | claude | In Progress | feature/framework-keyed-past-scripts |
