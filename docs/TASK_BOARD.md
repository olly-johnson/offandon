...
| BO-076 | Refine step rebuilt as a "Refine Studio". The Create-Script wizard's step 5 was a fire-and-forget textarea that re-ran the whole single-script generator and silently swapped the result. It is now a two-card surface: an editable script card (hook + body textareas, live word count, Copy, Save to library that persists the edited text) beside a chat card where the creator discusses the script with an assistant. New pure `diffLines` (`src/lib/shared/text-diff.ts`, LCS line diff, Vitest-covered) powers an inline current-vs-proposed diff. New `ScriptRefineChat` engine (`src/engines/content/script-refine-chat.ts` + `script-refine-system-prompt.ts`) does one tool-enabled `chat()` round trip over `IChatLLMClient`: a `propose_script_edit` tool returns an anti-slop-validated `{hook, body, word_count, summary}` proposal, plain replies just discuss. Inline server action `refineScriptChatAction` (usage surface `script_refine`) threads Voice DNA + methodology context like the other wizard actions. When the bot proposes an edit the editor card shows the diff with Accept (applies it) / Reject (discards). TDD: text-diff + script-refine-chat (+ prompt) + refine-studio component tests. | claude | In Progress | feature/script-refine-chat |
