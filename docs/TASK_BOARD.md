...
| BO-063 | Per-video analysis. `competitor_media_analysis` table (mirrors `instagram_media_analysis`) + RLS + realtime + `delete_user_data`. `analyze-competitor-media` Inngest function reuses Deepgram + `MediaAnalyzer` against each reel; library stats are computed from the competitor's own view-count distribution. Auto fan-out per reel from `competitor-scrape-completed`. New `/research/[competitorId]` drill-in page renders a reels grid with thumbnails, engagement, and inline analysis (Analyzing... spinner clears via realtime). New `competitor_analysis` surface added to `api_usage` for /admin spend tracking. | claude | In Progress | feature/research-analyze |
